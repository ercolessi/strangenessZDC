...
void AliAnalysisTaskLeading::UserCreateOutputObjects()
{
  ...
  //trigger mask
   htriggerMask[0] = new TH1D("htriggerMask","Trigger mask (before the event selection)",32,0,32);
   ...
   htriggerMask[1] = new TH1D("htriggerMask_noMB","Trigger mask excluding MB events (before the event selection)",32,0,32);
   for(Int_t i=0;i<32;i++) {
  ...
  fHth1d[0] = fHTrigger;
  fHth1d[1] = fHpv;
  fHth1d[2] = htriggerMask[0];
  fHth1d[3] = htriggerMask[1];
...
//   fTevents->Branch("ZDCP2",&fZDCP2,"ZDCP2/F");
  ...
  // TList example
  // fList[0]->Add(htriggerMask[0]); // example to add an object to the list
...
void AliAnalysisTaskLeading::UserExec(Option_t *) 
{
  ...
  AliESDInputHandler* inputHandler=(AliESDInputHandler*)mgr->GetInputEventHandler();

  // flag
  Bool_t isSelected = inputHandler->IsEventSelected();
