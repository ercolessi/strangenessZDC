...
void AnalisiFinCentrBis() {

  const char *outputname = "LambdaPtSpectraBis.root";
  ...
  TF1 *ffit = new TF1("ffit","gaus(0)/sqrt(2*TMath::Pi())/[2]+pol4(3)",1.09,1.14);
  ...
  TF1 *fback = new TF1("fback","pol4(0)*(x < 1.11 || x > 1.125)",1.09,1.14);
  ...
  for (int j = 0 ; j < ctr ; j++) { //loop over 12 centralities
    ...
    for (int i=0; i<= nbins; i++) {
      ...
      //Fit
      hProjMinv->Fit(fback,"");
      ffit->FixParameter(3,fback->GetParameter(0));
      ffit->FixParameter(4,fback->GetParameter(1));
      ffit->FixParameter(5,fback->GetParameter(2));
      ffit->FixParameter(6,fback->GetParameter(3));
      ffit->FixParameter(7,fback->GetParameter(4));
      hProjMinv->Fit(ffit);
      ffit->SetParameter(0,0);

      if(hProjMinv->Integral() < 200){
        ffit->FixParameter(3,0);
        ffit->FixParameter(4,0);
        ffit->FixParameter(5,0);
        ffit->FixParameter(6,0);
        ffit->FixParameter(7,0);   
      }

      eyield[i] = sqrt((h->Integral(h->FindBin(1.1),h->FindBin(1.125))))/hPT[j]->GetBinWidth(i);
...
//      hProjMinv->Fit(ffit,"","",1.1,1.13);
      ...
      yield[i] = h->Integral(h->FindBin(1.1),h->FindBin(1.125))/hPT[j]->GetBinWidth(i);
