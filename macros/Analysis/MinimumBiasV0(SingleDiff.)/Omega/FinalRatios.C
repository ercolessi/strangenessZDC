...
void FinalRatios() {
  ...
  const char *outputname = "OmegaLevyTsallis.root";
  const char *spectrafile = "PtSpectraOmega.root";
  ...
  for (int j = 0 ; j < (ctr) ; j++) {   
    ...
    cout << " Integral = " << ffit[j]->Integral(1.0,10.)/ffit[j]->Integral(0.,100.) << endl;     
  ...
  hPTClone[0]->SetAxisRange(1E-9,0.1,"Y");
  hPTClone[0]->Draw();
  ffit[0]->Draw("SAME");
  for(int j=1;j < (ctr);j++){
