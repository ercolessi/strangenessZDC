...
void IntegrYields() {
  ...
  const char *outputname = "A_APOmegaLevyTsallis.root";
  const char *ParticleSpectraFile = "PtSpectraOmega.root";
  const char *AntiParticleSpectraFile = "../AntiOmega/PtSpectraAntiOmega.root";
  ...
  for (int j = 0 ; j < (ctr) ; j++) {   
    ...
    for (int i = 3; i <= hPT_P[j]->GetNbinsX(); i++){
  ...
  for (int j = 0 ; j < (ctr) ; j++) {   
    ...
    cout << " Integral = " << ffit_P[j]->Integral(1.0,7.)/ffit_P[j]->Integral(0.,100.) << endl;     

    yieldsPart[j] += ffit_P[j]->Integral(0.0,1.0) + ffit_P[j]->Integral(7.0,100.0);
  ...
  for (int j = 0 ; j < (ctr) ; j++) {   
    ...
    for (int i = 3; i <= hPT_AP[j]->GetNbinsX(); i++){
  ...
  for (int j = 0 ; j < (ctr) ; j++) {   
    ...
    cout << " Integral = " << ffit_AP[j]->Integral(1.0,7.)/ffit_AP[j]->Integral(0.,100.) << endl;     

    yieldsAntiPart[j] += ffit_AP[j]->Integral(0.0,1.0) + ffit_AP[j]->Integral(7.0,100.0);
