...
void AliV0Module::SetLowEnergyValue ( Double_t lLoEnergyBound       ) {
    //Lower boundary (inclusive) in integer number for mult selection.
    //Note: If in PbPb and you want, say, 10-20% centrality, set this
    //to 10.
    fLoEnergyBound = lLoEnergyBound;
}

void AliV0Module::SetHighEnergyValue ( Double_t lHiEnergyBound       ) {
    //Lower boundary (inclusive) in integer number for mult selection.
    //Note: If in PbPb and you want, say, 10-20% centrality, set this
    //to 10.
    fHiEnergyBound = lHiEnergyBound;
}
...
void AliV0Module::DoAnalysis() {
    ...
    ********************************************************/
    ...
    Float_t fZDCpercentile = 0;
    Bool_t fEvSel_TriggeredCINT5 = 0;
    Long_t lNInelasticEvents = 0;

    cout<<"--------------- Open Real Data File --------------------"<<endl;
    TFile* file = TFile::Open(fRealDataFile, "READ");
	TList* v0list  = (TList*)file->Get("PWGLF_StrVsMult/cList");
    file->cd("PWGLF_StrVsMult");
    file->ls();
    if(fListName == "WithImprovedDCA") fListName = "ImprovedDCA";
    TTree* lTree;
    TTree* lTreeEvent;
    ...
    lTreeEvent->Print();
    //Variables from Tree Events
    lTreeEvent->SetBranchAddress("fCentrality",&fCentrality);
    lTreeEvent->SetBranchAddress("fZDCpercentile",&fZDCpercentile);

    cout<<"--------------------------------------------------------"<<endl;
    cout<<" Will now loop over events, please wait..."<<endl;

    Long_t lNInelasticGreaterThanZeroRefMult = 0;
    if( fPerformMultiplicityStudy == kTRUE ) {
        ...
        for(Long_t iEv = 0; iEv<lTreeEvent->GetEntries(); iEv++) {
            ...
            if ( iEv % ( lTreeEvent->GetEntries() / 10 ) == 0 ) cout<<" At Event "<<iEv<<" out of "<<lTreeEvent->GetEntries()<<endl;
                if(fCentrality >= fLoMultBound && fCentrality <= fHiMultBound) {
                    if (fZDCpercentile >= fLoEnergyBound && fZDCpercentile <= fHiEnergyBound) {
                        lNInelasticEvents++;
                    }
	   	        }
            }
        cout<<" Number of events, this multiplicity....: "<<lNInelasticEvents <<endl;
        cout<<"--------------------------------------------------------"<<endl;
    ...
    Float_t lZDCEnergy = 0;
    ///
    Float_t fTreeVariablePtLegPos; 
    Float_t fTreeVariablePtLegNeg;
    ULong64_t PosTrackStatus;
    ULong64_t NegTrackStatus;
    ULong64_t lITSrefitPos;
    ULong64_t lITSrefitNeg;
    Float_t lPtXv0Pos;
    Float_t lPtYv0Pos;
    Float_t lPtZv0Pos;
    Float_t lPtXv0Neg;
    ...
    lTree->SetBranchAddress("fTreeVariableNegTOFExpTDiff",&fTreeVariableNegTOFExpTDiff);
    lTree->SetBranchAddress("fTreeVariablePosTOFExpTDiff",&fTreeVariablePosTOFExpTDiff);
    lTree->SetBranchAddress("fTreeVariableNegEta",&lNegEta);
    lTree->SetBranchAddress("fTreeVariablePt",&lPt);
    lTree->SetBranchAddress("fTreeVariablePosTrackStatus",&PosTrackStatus); 
    lTree->SetBranchAddress("fTreeVariableNegTrackStatus",&NegTrackStatus);
    ...
    //--- Multiplicity Variable ----------------------------------------
    lTree->SetBranchAddress("fTreeVariableCentrality",&fCentrality);  
    lTree->SetBranchAddress("fZDCpercentile",&fZDCpercentile);    
    ...
    for(Long_t icand = 0; icand<lNCandidates; icand++) {
        ...
        lZDCEnergy = (Double_t) fZDCpercentile;
      
		ITSrefitAllPtOneLeg = ((NegTrackStatus & kITSrefit) || (PosTrackStatus & kITSrefit));
		TOFmatchAllPtOneLeg = (fTreeVariableNegTOFExpTDiff !=-2500) || (fTreeVariablePosTOFExpTDiff !=-2500);
        ...
        if(TMath::Abs(lRap)<fRapidityBoundary &&
                ...
                ( //official response code
                    ...
                    ( fWhichParticle == "K0Short"
                      && TMath::Abs(lNSigmasPosPion)   < fCutTPCPIDNSigmas
                      && TMath::Abs(lNSigmasNegPion)   < fCutTPCPIDNSigmas) ) 
	   			&&  ( fRequestITSTOF == -1 &&  (ITSrefitAllPtOneLeg || TOFmatchAllPtOneLeg) )
            ...
            if(lMultiplicity>=fLoMultBound &&  lMultiplicity<fHiMultBound)
            lHistoSelectedV0[lWeAreAtBin]->Fill(lInvariantMass); //fill with specific inv mass
                if(fFillQA){
		   ...
		   lCosPointingAngle->Fill(lV0CosinePointingAngle);
			...
			lHistoNsigmaTPCpos->Fill(lPt, lNSigmasPosProton); lHistoNsigmaTPCneg->Fill(lPt, lNSigmasNegPion);
			}
                   if(fWhichParticle == "AntiLambda") { 	
			lHistoNsigmaTPCpos->Fill(lPt, lNSigmasPosPion); lHistoNsigmaTPCneg->Fill(lPt, lNSigmasNegProton);
			}
                   if(fWhichParticle == "K0Short") { 
			lHistoNsigmaTPCpos->Fill(lPt, lNSigmasPosPion); lHistoNsigmaTPCneg->Fill(lPt, lNSigmasNegPion);
			}
		}
          }
    }
        ...
        if(TMath::Abs(lRap)<fRapidityBoundary &&
                ...
                ( //Multiplicity Switch
                    ...
                    (fPerformMultiplicityStudy == kTRUE &&  //inside mult bin
                     lZDCEnergy>=(fLoEnergyBound+0.000001) &&
                     lZDCEnergy<=(fHiEnergyBound-0.000001) &&
                     lMultiplicity>=(fLoMultBound+0.000001) &&
                     lMultiplicity<=(fHiMultBound-0.000001))  )
	   		&& 	( fRequestITSTOF == -1 &&  (ITSrefitAllPtOneLeg || TOFmatchAllPtOneLeg) )
         	) { // Start Entry Loop
            lWeAreAtBin = fHistPt->FindBin(lPt)-1;
            if(lWeAreAtBin == -1) lWeAreAtBin = 99; //UnderFlow, special treatment
           ...
           // if ( fWhichParticle == "Lambda" ) lProtonMomentumData[lWeAreAtBin]->Fill( 0.0 );
           // if ( fWhichParticle == "AntiLambda" ) lProtonMomentumData[lWeAreAtBin]->Fill( 0.0 );
        } // End Entry Loop
    }
    cout<<"--------------- Loop Completed -------------------------"<<endl;
    cout<<endl;
    ...
    for(Int_t ipoint = 0; ipoint<fptbinnumb; ipoint++) {
        ...
        cout<<" Number of events, this multiplicity....(check): "<<lNInelasticEvents <<endl; 
    ...
    TList* v0listMC  = (TList*)fileMC->Get("PWGLF_StrVsMult_MC/cList");
    ...
    Float_t lCreationRadius = -1;

    //Linking to Tree=================================================
    //--- Base Variables ----------------------------------------------
    
    lTreeMC->SetBranchAddress("fTreeVariablePosTrackStatus",&lITSrefitPos);
    lTreeMC->SetBranchAddress("fTreeVariableNegTrackStatus",&lITSrefitNeg);
    lTreeMC->SetBranchAddress("fTreeVariableNegTOFExpTDiff",&fTreeVariableNegTOFExpTDiff);
    lTreeMC->SetBranchAddress("fTreeVariablePosTOFExpTDiff",&fTreeVariablePosTOFExpTDiff);
    ...
    //--- Multiplicity ------------------------------------------------
    // lTreeMC->SetBranchAddress("fTreeVariableCentV0M",&lMultiplicity);
    //--- Multiplicity Variable ----------------------------------------
    lTreeMC->SetBranchAddress("fTreeVariableCentrality",&fCentrality);   
    lTreeMC->SetBranchAddress("fZDCpercentile",&fZDCpercentile); 
    //
    lTreeMC->SetBranchAddress("fTreeVariablePosInnerP",&fTreeVariablePtLegPos);
    lTreeMC->SetBranchAddress("fTreeVariablePosInnerP",&fTreeVariablePtLegNeg);
    // PID
    //--- TPC dEdx Variables ------------------------------------------
    ...
    for(Long_t icand = 0; icand<lNCandidatesMC; icand++) {
        ...
        if( fWhichEstimator == "V0M" ) lMultiplicity = (Double_t) fCentrality;
         lZDCEnergy = (Double_t) fZDCpercentile;
        
	    ITSrefitAllPtOneLeg = ((lITSrefitPos & kITSrefit) || (lITSrefitNeg & kITSrefit));
	    TOFmatchAllPtOneLeg = (fTreeVariableNegTOFExpTDiff !=-2500) || (fTreeVariablePosTOFExpTDiff !=-2500);
	   ...
	   //(
	    ...
	    //) &&
	   ( //Multiplicity Switch
	    (fPerformMultiplicityStudy == kFALSE || fUseIntegratedEfficiencies) || //No mult selection, or...
	    (fUseIntegratedEfficiencies==kFALSE && fPerformMultiplicityStudy == kTRUE &&  //inside mult bin
         lZDCEnergy>=(fLoEnergyBound+0.000001) &&
         lZDCEnergy<=(fHiEnergyBound-0.000001) &&
	     lMultiplicity>=(fLoMultBound+0.000001) &&
	     lMultiplicity<(fHiMultBound-0.000001) )
	  ...
	  if ( lPrimaryStatus == 1) {
	    ...
	    if ( fWhichParticle == "Lambda"  ) lProtonMomentumMC[lWeAreAtBin]->Fill( fTreeVariablePtLegPos );
	    if ( fWhichParticle == "AntiLambda" ) lProtonMomentumMC[lWeAreAtBin]->Fill( fTreeVariablePtLegNeg );
    ...
    if(fWhichParticle == "Lambda")
      fHistGen3D                    = (TH3D*)v0listMC->FindObject("fHistGeneratedPtVsYVsCentralityLambda");
    if(fWhichParticle == "AntiLambda")
      fHistGen3D                    = (TH3D*)v0listMC->FindObject("fHistGeneratedPtVsYVsCentralityAntiLambda");
    
    fHistGen = fHistGen3D->ProjectionX("fHistPtGen",
					      fHistGen3D->GetYaxis()->FindBin(-0.5+1e-2),  
					      fHistGen3D->GetYaxis()->FindBin(0.5-1e-2), 
					      fHistGen3D->GetZaxis()->FindBin(fLoMultBound),
					      fHistGen3D->GetZaxis()->FindBin(fHiMultBound));
    ...
    Double_t temppt; //  weight; // Int_t binMCw = -1;
     if(!fHistGen3D) { printf("Generated histogram does not exist!!! \n"); getchar();}
     if(!fHistGen) { printf("Generated histogram does not exist!!! \n"); getchar();}
    ...
    if( (fWhichParticle == "Lambda" || fWhichParticle == "AntiLambda") && fG3FlukaCorrOn) {
        ...
        TFile *fileCorrGeanT3FlukaPanos = new TFile("~/Scaricati/resultsSpectra/Results-AntiLambda-13TeV-V0M_0.0_100.0_UseMCRatioFD_WithV0refitAndImprovedDCA.root","READ");      
        fitGeant3FlukaCorr = (TF1*)fileCorrGeanT3FlukaPanos->Get("lGFCorrection/funcCorrAntiProtonFLUKA"); 
        ...
        for(Int_t ipoint = 0; ipoint<fptbinnumb; ipoint++) {
            ...
            cout<<"---> ["<<fptbinlimits[ipoint]<<" - "<<fptbinlimits[ipoint+1]
                <<" Corr " << (fitGeant3FlukaCorr->Eval( lProtonMomentum[ipoint]->GetMean()) )<<endl;
        }
    ...
    for(Int_t ipoint = 0; ipoint<fptbinnumb; ipoint++) {
            cout<<"---> ["<<fptbinlimits[ipoint]<<" - "<<fptbinlimits[ipoint+1]<<" GeV/c]\tEff.: "
            <<lEfficiency[ipoint]<<" +/- "<<lEfficiencyError[ipoint]<<endl;
            fHistEfficiency->SetBinContent(ipoint+1, lEfficiency[ipoint]);
            fHistEfficiency->SetBinError(ipoint+1, lEfficiencyError[ipoint]);
	}
        cout<<"--------------------------------------------------------"<<endl;
    ...
    if( fFDSwitch != "NoFD" && fWhichParticle != "K0Short") {
        ...
        //fileMCFD->cd("PWGLF_StrVsMult_MC");
        TList* v0listMCFD     = (TList*)fileMCFD->Get("PWGLF_StrVsMult_MC/cList");
        TTree* lTreeMCFD      = (TTree*)fileMCFD->Get("PWGLF_StrVsMult_MC/fTreeV0");
        TTree* lTreeEventMCFD = (TTree*)fileMCFD->Get("PWGLF_StrVsMult_MC/fTreeEvent");
        ...
        //Linking to Tree=================================================
        //--- Base Variables ----------------------------------------------
        lTreeMCFD->SetBranchAddress("fTreeVariablePosTrackStatus",&lITSrefitPos);
		lTreeMCFD->SetBranchAddress("fTreeVariableNegTrackStatus",&lITSrefitNeg);
        lTreeMCFD->SetBranchAddress("fTreeVariableNegTOFExpTDiff",&fTreeVariableNegTOFExpTDiff);
        lTreeMCFD->SetBranchAddress("fTreeVariablePosTOFExpTDiff",&fTreeVariablePosTOFExpTDiff);
        ...
        //---- Multiplicity info -------------------------------------------
        lTreeMCFD->SetBranchAddress("fTreeVariableCentrality",&lMultiplicity);
        //--- Armenteros-Podolansky ----------------------------------------
        lTreeMCFD->SetBranchAddress("fTreeVariablePosInnerP",&fTreeVariablePtLegPos);
        lTreeMCFD->SetBranchAddress("fTreeVariablePosInnerP",&fTreeVariablePtLegNeg);
        //--- TPC dEdx Variables ------------------------------------------
        ...
        Double_t xibinlimits[17] = {0.0, 0.6, 1.0, 1.2, 1.4, 1.6,  1.8, 2.0, 2.2,  2.5, 2.9, 3.4,  4.0,  5.0,  6.5,  8.,  10}; 
	  ...
	  if(TMath::Abs(lRap)<fRapidityBoundary &&
        ...
        //Filling histogram with original MC particles====================
        TH1D* fHistDummyV0FeedDown = 0x0;
        TH3D* f3dHistGenPtVsYVsMultV0FeedDown = 0x0;
        
        if (fWhichParticle == "Lambda" )
            f3dHistGenPtVsYVsMultV0FeedDown = (TH3D*)v0listMCFD->FindObject("fHistGeneratedPtVsYVsCentralityXiMinus");
        if (fWhichParticle == "AntiLambda" )
            f3dHistGenPtVsYVsMultV0FeedDown = (TH3D*)v0listMCFD->FindObject("fHistGeneratedPtVsYVsCentralityXiPlus");

        //Project MC histo for Xi
      	fHistDummyV0FeedDown = f3dHistGenPtVsYVsMultV0FeedDown->ProjectionX("fHistDummyV0FeedDown",
			       				f3dHistGenPtVsYVsMultV0FeedDown->GetYaxis()->FindBin(-0.5+1e-2),  
                                f3dHistGenPtVsYVsMultV0FeedDown->GetYaxis()->FindBin(+0.5-1e-2), 
			      				f3dHistGenPtVsYVsMultV0FeedDown->GetZaxis()->FindBin(0.0),
			     				f3dHistGenPtVsYVsMultV0FeedDown->GetZaxis()->FindBin(100.0));
        ...
        for(Int_t ipoint = 0; ipoint<fptbinnumb; ipoint++) {
            for(Int_t ixb = 0; ixb<xibinnumb; ixb++) {
                ...
                cout<<"---> ["<<fptbinlimits[ipoint]<<" - "<<fptbinlimits[ipoint+1]
                <<" GeV/c]\tFDCorr.: "<<lFeedDownEfficiencyGFCorrected[ipoint][ixb]
                <<" +/- "<<lFeedDownEfficiencyGFCorrectedError[ipoint][ixb]
                <<" Corr " << (fitGeant3FlukaCorr->Eval( lProtonMomentum[ipoint]->GetMean()) )<<endl;
        ...
        fLevyFitXiFeedDown = new TF1("LevyFitXiFeedDown",this ,&AliV0Module::MyLevyPtXi, 0.0,10,3, "AliV0Module","MyLevyPXi");
        TFile *fileXiSpectra;

        if (fWhichParticle == "Lambda") fileXiSpectra = new TFile("CorrSpectra-XiMinus_V0M0.0-100.0.root", "READ")   ; // xi spectra; errors tot: stat+syst
        else if (fWhichParticle =="AntiLambda") fileXiSpectra = new TFile("CorrSpectra-XiPlus_V0M0.0-100.0.root", "READ")   ;
        TH1F *XiSpectra = (TH1F*) ( fileXiSpectra->Get("hRawpT_0.0-100.0")   );
        TF1 *levyFit = (TF1*)(fileXiSpectra->Get("LevyFitXi"));
        ...
        //
        //fLevyFitXiFeedDownErr[0] = (TH1D*)fileXiSpectra->Get(Form("HistoFeeddownEnvelopeLow_%s_%1.1f_%1.1f",fWhichEstimator.Data(),0.0,1.0)); 
        //fLevyFitXiFeedDownErr[1] = (TH1D*)fileXiSpectra->Get(Form("HistoFeeddownEnvelopeHi_%s_%1.1f_%1.1f",fWhichEstimator.Data(),0.0,1.0));
        ////// 
       ...
       // cout<<"  (computed on the fly from <Nch> = "<<lAverageMult<<") "<<endl;
        cout<<"--------------------------------------------------------"<<endl;
        ...
        for(Int_t ixb = 0; ixb<xibinnumb; ixb++) {
           if(fFDSwitch == "DoubleChargedXi") lProducedXi[ixb] = lNInelasticEvents * 2.* (fLevyFitXiFeedDown->Integral(xibinlimits[ixb],xibinlimits[ixb+1]));
           else if(fFDSwitch == "UseMCRatio") lProducedXi[ixb] = lNInelasticEvents * 
          // XiSpectra->GetBinContent(ixb+1) * 
           	//XiSpectra->GetBinWidth(ixb+1);
            (fLevyFitXiFeedDown->Integral(xibinlimits[ixb],xibinlimits[ixb+1])); 
        }
        if(fWhichParticle == "Lambda")
		...
		cout<<"--------------------------------------------------------"<<endl;
        	...
        	cout<<"--------------------------------------------------------"<<endl;
                ...
                for(Int_t ixb = 0; ixb<xibinnumb; ixb++) {
            	if(fFDSwitch == "DoubleChargedXi") lProducedXiSyst[ixb] = lNInelasticEvents * fLevyFitXiFeedDownErr[i]->GetBinContent(ixb+1)*2.;
            	else if(fFDSwitch == "UseMCRatio") lProducedXiSyst[ixb] = lNInelasticEvents * (fLevyFitXiFeedDownErr[i]->GetBinContent(ixb+1));
