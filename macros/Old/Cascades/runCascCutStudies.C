...
void runCascCutStudies(
  ...
  if( lCascType.Contains("Plus") ) {
      TFile* fGeantFluka = new TFile("~/Scaricati/resultsSpectra/Results-XiPlus-V0M-000to100_WithV0refitAndImprovedDCA.root", "READ");
  ...
  //Set data files
  TString lMCFilename = "/alistorage/effectiveenergy/OldMonteCarloMerged15f.root" ;
  TString lDataFilename  = "/alistorage/effectiveenergy/StrangenessFiles/Strangeness15f.root" ;//0x0;
  // 
  ...
  if( lWhichEstimator.Contains("V0") ) casc->SetOutputFile( Form("Results-%s-13TeV-%s_%03.0f_%03.0f_ZDC_000_100.root", lCascType.Data(), lWhichEstimator.Data(), 
    (lMultBoundLo), (lMultBoundHi)) );
  else                                 casc->SetOutputFile( Form("Results-%s-13TeV-%s_%03.0f_%03.0f_ZDC_000_100.root", lCascType.Data(), lWhichEstimator.Data(), (Int_t)(lMultBoundLo),      (Int_t)(lMultBoundHi))      );

  //Run Cut Studies
  if (kDoCutStudies == kTRUE){

    TString lSystFile = "Results-Systematics";
        if( lWhichEstimator.Contains("V0") ) lSystFile.Append( Form("-%s-13TeV-%s_%03.0f_%03.0f_ZDC_000_100-",lCascType.Data(),lWhichEstimator.Data(),(lMultBoundLo),(lMultBoundHi)) );
        else                                 lSystFile.Append( Form("-%s-13TeV-%s_%03.0f_%03.0f_ZDC_000_100-",lCascType.Data(),lWhichEstimator.Data(),(lMultBoundLo),(lMultBoundHi) ));
    ...
    for (int i = 0; i < npoints ; i++){
      casc->SetCutV0Radius(cut); casc->SetOutputFile( lSystFile + "V0Radius" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }
    ...
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutCascRadius(cut); casc->SetOutputFile( lSystFile + "CascRadius" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    } 
    ...
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutV0CosPA(cut); casc->SetOutputFile( lSystFile + "V0CosPA" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }
    ...
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutCascCosPA(cut); casc->SetOutputFile( lSystFile + "CascCosPA" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //DCANegToPV
    cout<<endl<<"---> Performing Systematics Studies: DCANegToPV"<<endl;
    low = 0;
    high = 0.4;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutDCANegToPV(cut); casc->SetOutputFile( lSystFile + "DCANegToPV"  + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //DCAPosToPV
    cout<<endl<<"---> Performing Systematics Studies: DCAPosToPV"<<endl;
    low = 0;
    high = 0.4;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutDCAPosToPV(cut); casc->SetOutputFile( lSystFile + "DCAPosToPV" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //DCABachToPV
    cout<<endl<<"---> Performing Systematics Studies: DCABachToPV"<<endl;
    low = 0;
    high = 0.3;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutDCABachToPV(cut); casc->SetOutputFile( lSystFile + "DCABachToPV" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }
    ...
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutDCAV0ToPV(cut); casc->SetOutputFile( lSystFile + "DCAV0ToPV"  + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

     //DCAV0Daughters
    cout<<endl<<"---> Performing Systematics Studies: DCAV0Daughters"<<endl;
    low = 0;
    high = 3.;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutDCAV0Daughters(cut); casc->SetOutputFile( lSystFile + "DCAV0Daughters" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

     //DCACascDaughters
    cout<<endl<<"---> Performing Systematics Studies: DCACascDaughters"<<endl;
    low = 0;
    high = 3.;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutDCACascDaughters(cut); casc->SetOutputFile( lSystFile + "DCACascDaughters" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //V0Mass
    cout<<endl<<"---> Performing Systematics Studies: V0Mass"<<endl;
    low = 0;
    high = 0.02;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutV0Mass(cut); casc->SetOutputFile( lSystFile + "V0Mass" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //TPCNClusters
    cout<<endl<<"---> Performing Systematics Studies: TPCNClusters"<<endl;
    low = 60;
    high = 140;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutLeastNumberOfClusters(cut); casc->SetOutputFile( lSystFile + "TPCNClusters" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //TPCPIDNSigmas
    cout<<endl<<"---> Performing Systematics Studies: TPCPIDNSigmas"<<endl;
    low = 0;
    high = 8;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutTPCPIDNSigmas(cut); casc->SetOutputFile( lSystFile + "TPCPIDNSigmas" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //ProperLifetime
    cout<<endl<<"---> Performing Systematics Studies: ProperLifetime"<<endl;
    low = 0;
    high = 30;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutProperLifetime(cut); casc->SetOutputFile( lSystFile + "ProperLifetime" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //SigExtBinCount
    cout<<endl<<"---> Performing Systematics Studies: SigExtBinCount"<<endl;
    low = 0.8;
    high = 7.;
    cut = low;
    for (int i = 0; i < npoints ; i++){
      casc->SetCutSigmaForSignalExtraction(cut); casc->SetOutputFile( lSystFile + "SigExtBinCount" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
      cut += (high-low)/npoints;
    }

    //CompetingSpecies
    if (lCascType == "OmegaMinus" || lCascType == "OmegaPlus"){
      cout<<endl<<"---> Performing Systematics Studies: CompetingSpecies"<<endl;
      low = 0;
      high = 0.1;
      cut = low;
      for (int i = 0; i < npoints ; i++){
        casc->SetCutCompetingSpecies(cut); casc->SetOutputFile( lSystFile + "CompetingSpecies" + Form("-CutStudies-%i.root",i+1) ); casc->DoAnalysis();
        cut += (high-low)/npoints;
      }
    }
    ...
    if( lWhichSystVar == "" )  casc->DoAnalysis();
        ...
        ////////////////////////////////////////////////////////////////////////////
        // SIGEXTBINCOUNT //////////////////////////////////////////////////////////
        if( lWhichSystVar == "SigExtBinCountCompetingSpecies" ) {
